...
func GetAllEngineers(db *sql.DB) ([]types.Engineer, error) {
	...
	query := "SELECT idengineer, name, email FROM engineers"
...
func GetEngineerByID(db *sql.DB, id int) (types.Engineer, error) {
	...
	query := `
		SELECT idengineer, name, email FROM engineers WHERE idengineer = $1
	`
	err := db.QueryRow(query, id).Scan(
		&engineer.ID, &engineer.Name, &engineer.Email)
	if err != nil {
...
func AddEngineer(db *sql.DB, engineer types.EngineerRequest) (int, error) {
	...
	`
	err := db.QueryRow(query, engineer.Name, engineer.Email).Scan(&newID)
	if err != nil {
...
func UpdateEngineer(db *sql.DB, engineer types.Engineer) error {
	query := `
		...
		WHERE idEngineer = $3
	`
	_, err := db.Exec(query, engineer.Name, engineer.Email, engineer.ID)
	if err != nil {
